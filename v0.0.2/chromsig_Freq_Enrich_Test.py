...
def write_master_result(out_read_list, out_name):
    ...
    with open(out_name, 'a') as file1:
        header = ['READ_ID', 'Start Coord', 'End Coord', 'Category', 'Obs', 'rawpval1', 'adjpval1', 'decis1', 'rawpval2', 'adjpval2', 'decis2']
...
if __name__ == '__main__':
    ...
    subset_reads, cnt_all, cnt_subset, read_span = read_gems(directory, file_name, chr_name)
    ...
    #subset_reads = [x for x in in_gems if chr_name == x[1].split(":")[0]]
    ...
    for k in range(len(subset_reads)):
        ...
        out_gems.append([read_id, subset_reads[read_id][0][1], subset_reads[read_id][-1][2], 'Orig', round(enr,1), round(pval,3)])
