...
{
    echo -e "usage:
    bash multijob_bam2freq_enrich_bg.sh --bam bam_file --dir directory --r ref --fdr fdr --num samp_size --type str_type"
}
...
then
    ...
    jid1=$(sbatch makebed.sh --bp ${bam_prefix} --bam ${bam_file} --dir ${directory} | awk '{print $4}')
fi
...
then
    ...
    then
        ...
        jid4=$(sbatch --dependency=afterok:$jid3 denoise.sh --bg ${std_bg} --bed ${std_bed} --dir ${directory} --lib ${lib_name} --r ${ref} --fdr ${fdr} --num ${samp_size} --type ${str_type} | awk '{print $4}')
    else
        cd ..
        jid4=$(sbatch denoise.sh --bg ${std_bg} --bed ${std_bed} --dir ${directory} --lib ${lib_name} --r ${ref} --fdr ${fdr} --num ${samp_size} --type ${str_type} | awk '{print $4}')
    fi
    cd ${output_dir}
    ...
    then
        ...
        then
            ...
            jid8=$(sbatch --dependency=afterok:$jid7 sicer_run.sh --od ${directory} --ref ${ref} --bedf ${bed_file} | awk '{print $4}')
        else
            cd ../..
            echo "Creating total score island file (bed already made)"
            jid8=$(sbatch sicer_run.sh --od ${directory} --ref ${ref} --bedf ${bed_file} | awk '{print $4}')
        fi
    fi
    cd ${directory}
    if [ ! -f "venn_fig.png" ]
    then
        ...
        then
            cd ..
            jid9=$(sbatch --dependency=afterok:$jid8 venn_diag.sh --od ${output_dir} --pref ${pass_score_pref} --bpref ${bam_prefix} | awk '{print $4}')
        else
            cd ..
            jid9=$(sbatch venn_diag.sh --od ${output_dir} --pref ${pass_score_pref} --bpref ${bam_prefix} | awk '{print $4}')
        fi
    fi
