...
function usage
{
    ...
    USAGE:
    sbatch denoise.sh --cd chromsig_dir --bg std_bg --bed std_bed --dir directory --lib lib_name --r ref --fdr fdr --num samp_size --type str_type --plot plot_hist --cov coverage
      ...
      ARGUMENTS:
        ...
        --cov   Minimum coverage required for a chromsome to be processed (ex.: For minimum 5% coverage, input should be --cov 0.05).
...
echo "Denoising: $std_bed"

cd ${directory}

# determining amount of coverage

# Merge bed file to get an accurate idea of read coverage -- otherwise the coverage may be overcounted due to overlaps between reads.

bedtools merge -i ${std_bed} > "${lib_name}_merge_result.bed"

# Generated a summary of the coverage of each chromosome in the merged file

bedtools summary -i "${lib_name}_merge_result.bed" -g "${ref}.chrom.sizes" | column -t > "${lib_name}_coverage_summary.txt"

# Divide the number of basepairs covered by reads by the total number of basepairs for each chromosome -- this gives us a fraction representing the coverage.
...
# Form an array containing all chromosomes and corresponding coverages.
...
# chrY always has extremely low coverage so we automatically eliminate this.
chromtoremove="chrY"

# Using the coverage array, eliminate any chromosomes with <coverage percentage from consideration.
chromslst=()
...
for item in "${!chromslst[@]}"; do
  ...
  then
    ...
    then
      ...
      if (( $(echo "$num_to_compare > $coverage" | bc -l) )); then
...
# Using the final count of chromosomes to be processed, run the Python file containing the denoising algorithm on each chromsome individually
# The SLURM array allows us to run all chromosomes in parallel
