...
{
    echo -e "DESCRIPTION:
    The purpose of this command is to denoise 1-dimensional chromatin profiles. This data must be in single-end or paired-end format. The command takes a .bam or .bed file as input, and outputs 
    ...
    USAGE:
    sbatch multijob_bam2freq_enrich_bg.sh --bamorbed input_file --dir data_directory --r ref --fdr fdr --num samp_size --type str_type --cov coverage --lib lib_name --plot true --modload true
        ...
        EXAMPLE:
            sbatch multiscript_bam2freq_enrich_bg.sh --bamorbed GM12878_ATAC-seq_ENCFF415FEC.bam --dir /nfs/turbo/umms-minjilab/njgupta/chromsig/ATAC-seq_PE/ --r hg38 --fdr 0.1 --num 5000 --type PE --cov 0.05 --modload true"
}
...
then
    ...
    then
        cd ${chromsig_dir}
        jid4=$(sbatch --dependency=afterok:$jid3 denoise.sh --cd ${chromsig_dir} --bg ${std_bg} --bed ${std_bed} --dir ${directory} --lib ${lib_name} --r ${ref} --fdr ${fdr} --num ${samp_size} --type ${str_type} --plot ${plot_hist} --cov ${coverage} | awk '{print $4}')
    else
        cd ${chromsig_dir}
        jid4=$(sbatch denoise.sh --cd ${chromsig_dir} --bg ${std_bg} --bed ${std_bed} --dir ${directory} --lib ${lib_name} --r ${ref} --fdr ${fdr} --num ${samp_size} --type ${str_type} --plot ${plot_hist} --cov ${coverage} | awk '{print $4}')
    fi
    cd ${output_dir}
    ...
    then
        ...
        then
            ...
            cd ${chromsig_dir}
            pwd
            jid5=$(sbatch --dependency=afterok:$jid4 pileup2bedgraph.sh --od ${output_dir} --dir ${directory} --lib ${lib_name} --r ${ref} --fdr ${fdr} --num ${samp_size} --type ${str_type} --bed ${std_bed} --cov ${coverage} | awk '{print $4}')
        else
            echo "Creating pass bedgraphs right away"
            cd ${chromsig_dir}
            jid5=$(sbatch pileup2bedgraph.sh --od ${output_dir} --dir ${directory} --lib ${lib_name} --r ${ref} --fdr ${fdr} --num ${samp_size} --type ${str_type} --bed ${std_bed} --cov ${coverage} | awk '{print $4}')
        fi
    fi
    cd ${output_dir}
