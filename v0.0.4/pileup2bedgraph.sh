...
{
    echo -e "usage:
    sbatch pileup2bedgraph.sh --od output_dir --dir directory --lib lib_name --r ref --fdr fdr --num samp_size --type str_type --bed std_bed --cov coverage"
}
...
# In the same way we determined chromosome coverage in denoising.sh, determine coverage again here (using the files that have already been generated by denoising.sh)
...
# For each chromsome in consideration (coverage >input coverage percentage), convert pass pileup and fail pileup into bedgraphs, running all chromosomes in parallel
